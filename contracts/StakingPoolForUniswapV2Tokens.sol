 ...
 * @notice It is a staking pool for Uniswap V2 LP tokens (stake Uniswap V2 LP tokens -> get IMINTY).
 */
contract StakingPoolForUniswapV2Tokens is Ownable, Pausable, ReentrancyGuard {
    ...
    // IMINTY token (token distributed)
    IERC20 public immutable IMINTYToken;

    // The staked token (i.e., Uniswap V2 WETH/IMINTY LP token)
    ...
    // Tokens distributed per block (in IMINTYToken)
     ...
     * @param _IMINTYToken reward token address
     * @param _rewardPerBlock reward per block (in IMINTY)
     * @param _startBlock start block
     * @param _endBlock end block
     */
    constructor(
        address _stakedToken,
        address _IMINTYToken,
        uint256 _rewardPerBlock,
        uint256 _startBlock,
        uint256 _endBlock
    ) {
        stakedToken = IERC20(_stakedToken);
        IMINTYToken = IERC20(_IMINTYToken);
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        if (userInfo[msg.sender].amount > 0) {
            ...
            if (pendingRewards > 0) {
                IMINTYToken.safeTransfer(msg.sender, pendingRewards);
            }
        }
     ...
     * @param amount amount to withdraw (in IMINTYToken)
     * @dev Only callable by owner.
     */
    function adminRewardWithdraw(uint256 amount) external onlyOwner {
        IMINTYToken.safeTransfer(msg.sender, amount);
