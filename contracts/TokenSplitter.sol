...
/**
 * @title TokenSplitter
 * @notice It splits IMINTY to team/treasury/trading volume reward accounts based on shares.
 */
contract TokenSplitter is Ownable, ReentrancyGuard {
    ...
    IERC20 public immutable IMINTYRareToken;

    // Total IMINTY tokens distributed across all accounts
    uint256 public totalTokensDistributed;

    mapping(address => AccountInfo) public accountInfo;

    event NewSharesOwner(
     ...
     * @param _IMINTYRareToken address of the IMINTY token
     */
    constructor(
        ...
        IMINTYRareToken = IERC20(_IMINTYRareToken);
    }

    /**
     * @notice Release IMINTY tokens to the account
     * @param account address of the account
     */
    function releaseTokens(address account) external nonReentrant {
        require(
            accountInfo[account].shares > 0,
            "Splitter: Account has no share"
        );

        // Calculate amount to transfer to the account
        uint256 totalTokensReceived = IMINTYRareToken.balanceOf(address(this)) +
        ...
        IMINTYRareToken.safeTransfer(account, pendingRewards);
        ...
        onlyOwner
    {
        ...
        require(
            accountInfo[_newRecipient].shares == 0,
            "Owner: New recipient has existing shares"
        );

        // Copy shares to new recipient
        accountInfo[_newRecipient].shares = accountInfo[_currentRecipient]
            .shares;
        accountInfo[_newRecipient].tokensDistributedToAccount = accountInfo[
            _currentRecipient
    ...
    /**
     * @notice Retrieve amount of IMINTY tokens that can be transferred
     * @param account address of the account
     */
